function testLatDerivative(testCase)
  numPoints = 100;
  hxx = 10;
  YEAR = 2012;
  xlat = 40;
  ...
  for ex = minEx:incEx:maxEx
    del = abs(xlat)*10.0^(-ex);
    alts(i) = del;
    %[TL,ciractx] = ciractx.GLOBE7(iyd,sec,xlat+del,xlon,stl,f107a,f107,ap,ciractx.PT);
    %[ D,T,ciractx ] = ciractx.GTS7(iyd,sec,hxx,xlat+del,xlon,stl,f107a,f107,ap,mass,0.0);
    [ D,T,ciractx ] = ciractx.GTD7(iyd,sec,hxx,xlat+del,xlon,stl,f107a,f107,ap,mass,0.0);
    %TL = ciractx.DM28;
    dtemps(i) = T(CIRA.TemperatureIndex(CIRA.DerLat));
    d2temps(i) = (T(CIRA.TemperatureIndex(CIRA.Der0))-T0(CIRA.TemperatureIndex(CIRA.Der0)))/del;
    ddenss(i) = D(CIRA.N2_LATGDENS);
    d2denss(i) = (D(CIRA.N2_DENS)-D0(CIRA.N2_DENS))/del;
    %d2denss(i) = (D(CIRA.N2_DENS)-D0(CIRA.N2_DENS))/del-D(CIRA.N2_LATGDENS);
    %d2denss(i) = (TL(K,CIRA.Der0)-TL0(K,CIRA.Der0))/del-TL0(K,CIRA.DerLat);
    i = i + 1;
  end
  figure;
  ...
  title('Latitude Derivative of Density');
  xlabel('fraction from latitude');
