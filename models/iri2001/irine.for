C IRINE.FOR, Version 2001.03, July 11, 2001
C 
...
C faster.
C
C includes subroutines IRI_SUB_NE to compute IRI Ne for specified
C location, date, time, and altitude range and subroutine IRI_WEB_NE to 
...
c month, day of month, day of year, or hour (UT or LT). IRI_WEB_NE requires
C IRI_SUB. Both subroutines require linking with the following library
C files IRIFUN.FOR, IRITEC.FOR, IRIDREG.FOR, CIRA86.FOR, IGRFSUB.FOR
C
C !!!!!! UNIT NUMBERS USED FOR READING OF COEFFICIENTS:
C The program uses the I/O units 10 (CCIR/URSI maps for foF2 and M3000F2),
C 12 (IG12 ionospheric indices and Rz12 solar indices), and 13 (3-hourly
C Ap magnetic indices). Only Unit 10 is accessed from IRISUB.FOR, 12 and 13
C are accessed from subroutines within IRIFUN.FOR.
C
C*****************************************************************
...
C-Version-MM/DD/YY-----------------------------------------------------------
C 2000.01 05/09/00 B0_98 replaces B0_TAB and B1 varies from 1.9/day to 2.6/night
...
C 2001.01 03/17/01 include Truhlik-Triskova Te model and IGRF
C 2001.02 05/07/01 include Fuller-Rowell-Condrescu storm model 
C 2001.03 07/11/01 LATI instead of LAT1 in F00 call (K. Torkar)
C 2001.04 09/18/01 correct comp. of foF2 for Rz12 user input (P. Wilkinson)
C 2001.04 09/19/01 call APF only if different date and time (P. Webb)
C 2001.04 09/24/01 add , in declaration of RZIN0 as logical variable (P. Webb)
C
C*****************************************************************
...
C         14         hmF1 from model        hmF1 - user input (only Lay version)
...
C         19         F1 probability model   critical solar zenith angle (old)
C         20         standard F1 	    standard F1 plus L condition
C	  21         ion drift not computed ion drift computed
C         22         ion densities in %     ion densities in cm-3
C         23         Te_tops (Aeros,ISIS)   Te_topside (Intercosmos)
C         24         D-region: IRI-95       Special: 3 D-region models
C         25         F107D=COV		    F107D user input (oarr(41))
C         26	     foF2 storm model	    no storm updating
C         27-30      unused
C      ---------------------------------------------------------------------
C
...
C                            available. If jf(23)=.false. then input
C                            for Ne(300km), Ne(550km)/m-3, oarr(17)=-1
...
C               OARR(39) = rsat (IG12)        oarr(40) = F1 probability (F1PB)
...
c      CHARACTER FILNAM*53
     ...
     &  AMP(4),HXL(4),SCL(4),B0B1(5),XSM(4),MM(5),DTI(4),
     &  AHH(7),STTE(6),DTE(5),ATE(7),TEA(6),XNAR(3),
     &  param(2),OUTF(20,100),OARR(50),
     &  PG1O(80),PG2O(32),PG3O(80),PF1O(12),PF2O(4),PF3O(12),
     &  HO(4),MO(5),DDO(4),HO2(2),MO2(3),DO2(2),DION(7)
      LOGICAL    EXT,SCHALT,NIGHT,TECON(3),sam_mon,sam_yea,
     &  sam_date,sam_ut,
     &  F1REG,FOF2IN,HMF2IN,URSIF2,LAYVER,DY,GULB0,DREG,rzino,
     &  FOF1IN,HMF1IN,FOEIN,HMEIN,RZIN,sam_doy,F1_OCPRO,F1_L_COND,
     &  NODEN,NOTEM,NOION,TENEOP,OLD79,JF(30),URSIFO
...
5592    continue
	sam_mon=(month.eq.montho)
        ...
        sam_ut=(ut.eq.ut0)
        if(sam_date.and..not.rzin.and..not.rzino) goto 2910
...
C
      ...
      IF(URSIF2.NEQV.URSIFO) GOTO 7797
        if(.not.rzin.and..not.rzino) then
...
104   	FORMAT('/home/ingemar/gup/dun/iri2001/ccir',I2,'.asc')
...
1144  	  FORMAT('/home/ingemar/gup/dun/iri2001/ursi',I2,'.asc')
c-web- special for web-version:
...
c stormtime updating
	if(jf(26)) then
		kut=int(ut)
		if (sam_date.and.sam_ut) goto 2819
...
c
	...
	if(f1_ocpro) then
		f1pb = f1_prob(xhi,mlat,rssn,f1_l_cond)
		if((fof1in).or.(f1pb.ge.0.5)) then 
			f1reg=.true.
		else
			f1reg=.false.
		endif
	else
		if(fof1in.or.((.not.night).and.(fof1.gt.0.0))) then
			f1reg=.true.
			f1pb = 1.0 
		else
			f1reg=.false.
...
C !!!!!!!!!! Interkosmos model !!!!!!!!!!!!!!!
C !!!!!!!!!! F10.7 effect included, seasonal effect not included
	dimo=0.311653
	icd=1
	isa=0
	if(height.le.ahh(4)) then
		TEH=ELTE(HEIGHT)
		goto 2918
		endif
	if(height.lt.550.) then
		TEH1=ELTE(ahh(4))
ccc        	call igrf_sub(lati,longi,ryear,height,
ccc     &          	xl,icode,dipl,babs)
		if(xl.gt.30.) xl=30.
ccc		call elteik(icd,isa,0,invdip,xl,dimo,babs,
ccc     &			dipl,hour,550.,daynr,f107d,teh2,sdte)
ccc		teh=teh1+(teh2-teh1)/(550.-ahh(4))*(height-ahh(4))
		goto 2918
		endif
ccc        call igrf_sub(lati,longi,ryear,height,
ccc     &          xl,icode,dipl,babs)
	if(xl.gt.30.) xl=30.
ccc	call elteik(icd,isa,0,invdip,xl,dimo,babs,dipl,hour,
ccc     &		height,daynr,f107d,teh,sdteh)

2918	IF(TIH.LT.TNH) TIH=TNH
