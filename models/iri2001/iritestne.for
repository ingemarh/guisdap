c iritestne.for, version 2001.02, July 11, 2001
c
c test program for the iri_web_ne subroutine
c
c corrections
c-version-mm/dd/yy-------------------------------------------------
...
1       print *,'jmag(=0/1,geog/geom),lati/deg,long/deg'
     ...
     &		't,t,t,t,t,t,t,t,t,t'
        ...
        if(jchoice.eq.0) then
          jf(2)=.false.               ! no Te,Ti computation
          jf(3)=.false.               ! no Ni computation
...
c
     ...
     &      .not.jf(15).or..not.jf(16)) then
          if(.not.jf(8)) jf(26)=.false.    ! storm model off, if user input
          var=vbeg
          i=1
...
c
      ...
      PIKTAB=0
        IF(IVAR.NE.1) THEN
                ...
                ENDIF
        IF(PIKTAB.GT.1) THEN
          WRITE(7,8194) ITEXT(IVAR),pna(pad1),pna(pad2),pna(pad3),
     &          xtex,uni(pad1),uni(pad2),uni(pad3)
