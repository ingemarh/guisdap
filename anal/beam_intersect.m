function [gg,d]=beam_intersect(T,Tdir,R,Rdir)
% inputs location [lat lon h] and directions [az,el] of Tx and Rx
gcA=[gg2gc(T);gg2gc(R)];
h0=300; %First guess
for i=1:2
  ggTd=loc2gg(T,[Tdir([2 1]) T(3)+h0]);
  ggRd=loc2gg(R,[Rdir([2 1]) R(3)+h0]);
  gcB=[gg2gc(ggTd);gg2gc(ggRd)];
  [P,d]=lineIntersect3D(gcA,gcB);
  gg=gc2gg(P);
  h0=gg(3);
end
