% Function version, CFE
% modified 2019-04-23, RS
% ...
% modified 2019-10-10, RS
...
if nargin<2
    error('Experiment day and output path needed.')
end
if ~exist(datapath)
    mkdir(datapath);
end
...
%addpath('/data/gup/guisdap9/anal');
%addpath /home/rikard/matlab/Guisdap/guisdap9/anal
start_GUP;
...
if isempty(image_filelist)
    ...
    b = 1;
%     cmd1 = 'source ~/python/venv/bin/activate';
%     cmd2 = ['LD_LIBRARY_PATH="" python3 /home/rikard/matlab/Guisdap/figuresorter/slapakreadinfo.py ' dirpath];
%     [status, result] = system([cmd1,';',cmd2])
end
...
if ~isempty(hdf5check)
    ...
    for ii = 1:length(hdf5check)
        ...
        if ~isletter(hdf5_filename(1))
            hdf5_file{n} = fullfile(dirpath,'overview',hdf5_filename);          % old hdf5-files starting with a number
            n = n+1;
        else
            hdf5letter_file{m} = fullfile(dirpath,'overview',hdf5_filename);    % old hdf5-files starting with a letter
            m = m+1;
        end
    end
end
...
for ii = 1:length(targz_file)
   ...
   % [~,storefolder,~] = fileparts(storepath);
    ...
    if isempty(gupfilecheck) && ~isempty(hdf5_file)
        mat2hdf5_ButMetaFromOld(hdf5_file{1})  % OBS!! takes the first hdf5-file in list, takes the experiment data and put in the hdf5-file just generated by mat2hdf5_instexpr_master.m
    end
