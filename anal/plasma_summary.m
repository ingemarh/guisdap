...
elseif plots==0 | plots==-2
 ...
 for g=1:dgates
  ...
  for j=1:nfreq
   ...
   if j==1
    text('Position',[p_time(end) ymin+(ymax-ymin)*nfreq/2],'VerticalAlignment','top',...
     'FontSize',12,'HorizontalAlignment','Center','Rotation',90,...
 ...
 colormap(vizu('myb'))
 axlogo=axes('Position',[.05 .93 .07 .07]); eiscatlogo(axlogo,.5)
