...
for ii = 1:length(targz_file)
    ...
    if isempty(gupfilecheck) && ~isempty(hdf5_file)
        metacompl(hdf5_file{1})  % OBS!! takes the first hdf5-file in list, takes the experiment data and put them in the hdf5-file just generated by mat2hdf5.m
    end
...
for ii = 1:length(hdf5_file)
    ...
    storepath = cedar2hdf5(OldHdf5File,datapath);
